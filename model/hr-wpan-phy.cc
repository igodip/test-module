...
const uint32_t HrWpanPhy::aMaxPhyPacketSize = 127; // max PSDU in octets
const uint32_t HrWpanPhy::aTurnaroundTime = 12;  // RX-to-TX or TX-to-RX in symbol periods

// IEEE802.15.4-2006 Table 2 in section 6.1.2 (kb/s and ksymbol/s)
// The index follows HrWpanPhyOption
const HrWpanPhyDataAndSymbolRates
HrWpanPhy::dataSymbolRates[7] = { { 20.0, 20.0},
                                  { 40.0, 40.0},
                                  { 250.0, 12.5},
                                  { 250.0, 50.0},
                                  { 100.0, 25.0},
...
// The index follows HrWpanPhyOption
const HrWpanPhyPpduHeaderSymbolNumber
HrWpanPhy::ppduHeaderSymbolNumbers[7] = { { 32.0, 8.0, 8.0},
                                          { 32.0, 8.0, 8.0},
                                          { 2.0, 1.0, 0.4},
                                          { 6.0, 1.0, 1.6},
                                          { 8.0, 2.0, 2.0},
...
HrWpanPhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::HrWpanPhy")
    ...
    .AddTraceSource ("TrxState",
                     ...
                     MakeTraceSourceAccessor (&HrWpanPhy::m_trxStateLogger),
                     "ns3::HrWpanPhy::StateTracedCallback")
    ...
    m_setTRXState ()
{
  ...
  m_signal = Create<HrWpanInterferenceHelper> (m_noise->GetSpectrumModel ());
  m_rxLastUpdate = Seconds (0);
  Ptr<Packet> none_packet = 0;
  Ptr<HrWpanSpectrumSignalParameters> none_params = 0;
...
HrWpanPhy::DoDispose (void)
{
  ...
  m_pdDataConfirmCallback = MakeNullCallback< void, HrWpanPhyEnumeration > ();
  m_plmeCcaConfirmCallback = MakeNullCallback< void, HrWpanPhyEnumeration > ();
  m_plmeEdConfirmCallback = MakeNullCallback< void, HrWpanPhyEnumeration,uint8_t > ();
  m_plmeGetAttributeConfirmCallback = MakeNullCallback< void, HrWpanPhyEnumeration, HrWpanPibAttributeIdentifier, HrWpanPhyPibAttributes* > ();
  m_plmeSetTRXStateConfirmCallback = MakeNullCallback< void, HrWpanPhyEnumeration > ();
  m_plmeSetAttributeConfirmCallback = MakeNullCallback< void, HrWpanPhyEnumeration, HrWpanPibAttributeIdentifier > ();

  SpectrumPhy::DoDispose ();
}

Ptr<NetDevice>
HrWpanPhy::GetDevice (void)
{
  NS_LOG_FUNCTION (this);
  return m_device;
}
...
void
HrWpanPhy::SetDevice (Ptr<NetDevice> d)
{
...
void
HrWpanPhy::SetMobility (Ptr<MobilityModel> m)
{
...
void
HrWpanPhy::SetChannel (Ptr<SpectrumChannel> c)
{
...
HrWpanPhy::GetRxSpectrumModel (void) const
{
  NS_LOG_FUNCTION (this);
  if (m_txPsd)
    {
...
void
HrWpanPhy::SetAntenna (Ptr<AntennaModel> a)
{
...
void
HrWpanPhy::StartRx (Ptr<SpectrumSignalParameters> spectrumRxParams)
{
  ...
  if (!m_edRequest.IsExpired ())
    {
      ...
      m_edPower.averagePower += HrWpanSpectrumValueHelper::TotalAvgPower (m_signal->GetSignalPsd (), m_phyPIBAttributes.phyCurrentChannel) * (now - m_edPower.lastUpdate).GetTimeStep () / m_edPower.measurementLength.GetTimeStep ();
  ...
  if ( lrWpanRxParams == 0)
    {
      ...
      if (!m_ccaRequest.IsExpired ())
        {
          double power = HrWpanSpectrumValueHelper::TotalAvgPower (m_signal->GetSignalPsd (), m_phyPIBAttributes.phyCurrentChannel);
  ...
  if (m_trxState == IEEE_802_15_4_PHY_RX_ON && !m_setTRXState.IsRunning ())
    {
      ...
      *interferenceAndNoise += *m_noise;
      double sinr = HrWpanSpectrumValueHelper::TotalAvgPower (lrWpanRxParams->psd, m_phyPIBAttributes.phyCurrentChannel) / HrWpanSpectrumValueHelper::TotalAvgPower (interferenceAndNoise, m_phyPIBAttributes.phyCurrentChannel);
...
void
HrWpanPhy::CheckInterference (void)
{
  // Calculate whether packet was lost.
  HrWpanSpectrumValueHelper psdHelper;
  Ptr<HrWpanSpectrumSignalParameters> currentRxParams = m_currentRxPacket.first;
      ...
      if (m_errorModel != 0)
        {
          ...
          *interferenceAndNoise += *m_noise;
          double sinr = HrWpanSpectrumValueHelper::TotalAvgPower (currentRxParams->psd, m_phyPIBAttributes.phyCurrentChannel) / HrWpanSpectrumValueHelper::TotalAvgPower (interferenceAndNoise, m_phyPIBAttributes.phyCurrentChannel);
          ...
          HrWpanLqiTag tag (std::numeric_limits<uint8_t>::max ());
...
void
HrWpanPhy::EndRx (Ptr<SpectrumSignalParameters> par)
{
  NS_LOG_FUNCTION (this);

  Ptr<HrWpanSpectrumSignalParameters> params = DynamicCast<HrWpanSpectrumSignalParameters> (par);
  ...
  if (currentRxParams == params)
    {
      ...
      Ptr<HrWpanSpectrumSignalParameters> none = 0;
...
void
HrWpanPhy::PdDataRequest (const uint32_t psduLength, Ptr<Packet> p)
{
      ...
      if (m_trxState == IEEE_802_15_4_PHY_TX_ON)
        {
          ...
          m_pdDataRequest = Simulator::Schedule (txParams->duration, &HrWpanPhy::EndTx, this);
  ...
  if (m_trxState == IEEE_802_15_4_PHY_RX_ON || m_trxState == IEEE_802_15_4_PHY_BUSY_RX)
    {
      ...
      m_ccaRequest = Simulator::Schedule (ccaTime, &HrWpanPhy::EndCca, this);
    }
  else
    {
  ...
  if (m_trxState == IEEE_802_15_4_PHY_RX_ON || m_trxState == IEEE_802_15_4_PHY_BUSY_RX)
    {
      ...
      m_edRequest = Simulator::Schedule (m_edPower.measurementLength, &HrWpanPhy::EndEd, this);
    }
  else
    {
      HrWpanPhyEnumeration result = m_trxState;
...
void
HrWpanPhy::PlmeGetAttributeRequest (HrWpanPibAttributeIdentifier id)
{
  NS_LOG_FUNCTION (this << id);
  HrWpanPhyEnumeration status;
  ...
  if (!m_plmeGetAttributeConfirmCallback.IsNull ())
    {
      HrWpanPhyPibAttributes retValue;
      memcpy (&retValue, &m_phyPIBAttributes, sizeof(HrWpanPhyPibAttributes));
...
// Section 6.2.2.7.3
void
HrWpanPhy::PlmeSetTRXStateRequest (HrWpanPhyEnumeration state)
{
      ...
      if ((m_trxState == IEEE_802_15_4_PHY_BUSY_RX) || (m_trxState == IEEE_802_15_4_PHY_RX_ON))
        {
          ...
          m_setTRXState = Simulator::Schedule (setTime, &HrWpanPhy::EndSetTRXState, this);
          return;
        }
...
bool
HrWpanPhy::ChannelSupported (uint8_t channel)
{
...
void
HrWpanPhy::PlmeSetAttributeRequest (HrWpanPibAttributeIdentifier id,
                                    HrWpanPhyPibAttributes* attribute)
{
  NS_LOG_FUNCTION (this << id << attribute);
  NS_ASSERT (attribute);
  HrWpanPhyEnumeration status = IEEE_802_15_4_PHY_SUCCESS;
...
void
HrWpanPhy::SetPdDataIndicationCallback (PdDataIndicationCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
HrWpanPhy::SetPdDataConfirmCallback (PdDataConfirmCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
HrWpanPhy::SetPlmeCcaConfirmCallback (PlmeCcaConfirmCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
HrWpanPhy::SetPlmeEdConfirmCallback (PlmeEdConfirmCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
HrWpanPhy::SetPlmeGetAttributeConfirmCallback (PlmeGetAttributeConfirmCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
HrWpanPhy::SetPlmeSetTRXStateConfirmCallback (PlmeSetTRXStateConfirmCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
HrWpanPhy::SetPlmeSetAttributeConfirmCallback (PlmeSetAttributeConfirmCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
HrWpanPhy::ChangeTrxState (HrWpanPhyEnumeration newState)
{
...
bool
HrWpanPhy::PhyIsBusy (void) const
{
...
void
HrWpanPhy::CancelEd (HrWpanPhyEnumeration state)
{
  NS_LOG_FUNCTION (this);
...
void
HrWpanPhy::EndEd (void)
{
  NS_LOG_FUNCTION (this);

  m_edPower.averagePower += HrWpanSpectrumValueHelper::TotalAvgPower (m_signal->GetSignalPsd (), m_phyPIBAttributes.phyCurrentChannel) * (Simulator::Now () - m_edPower.lastUpdate).GetTimeStep () / m_edPower.measurementLength.GetTimeStep ();
...
void
HrWpanPhy::EndCca (void)
{
  NS_LOG_FUNCTION (this);
  HrWpanPhyEnumeration sensedChannelState = IEEE_802_15_4_PHY_UNSPECIFIED;
...
Time
HrWpanPhy::CalculateTxTime (Ptr<const Packet> packet)
{
...
double
HrWpanPhy::GetDataOrSymbolRate (bool isData)
{
...
Time
HrWpanPhy::GetPpduHeaderTxTime (void)
{
  NS_LOG_FUNCTION (this);
...
void
HrWpanPhy::SetTxPowerSpectralDensity (Ptr<SpectrumValue> txPsd)
{
...
void
HrWpanPhy::SetNoisePowerSpectralDensity (Ptr<const SpectrumValue> noisePsd)
{
...
HrWpanPhy::GetNoisePowerSpectralDensity (void)
{
  NS_LOG_FUNCTION (this);
  return m_noise;
}
...
void
HrWpanPhy::SetErrorModel (Ptr<HrWpanErrorModel> e)
{
...
Ptr<HrWpanErrorModel>
HrWpanPhy::GetErrorModel (void) const
{
  NS_LOG_FUNCTION (this);
  return m_errorModel;
}
...
uint64_t
HrWpanPhy::GetPhySHRDuration (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT (m_phyOption < IEEE_802_15_4_INVALID_PHY_OPTION);
...
double
HrWpanPhy::GetPhySymbolsPerOctet (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT (m_phyOption < IEEE_802_15_4_INVALID_PHY_OPTION);
...
int64_t
HrWpanPhy::AssignStreams (int64_t stream)
{
  NS_LOG_FUNCTION (this);
