...
#include "hr-wpan-interference-helper.h"
...
} HrWpanPhyPpduHeaderSymbolNumber;
 ...
 */
typedef Callback< void, HrWpanPhyEnumeration > PdDataConfirmCallback;
 ...
 */
typedef Callback< void, HrWpanPhyEnumeration > PlmeCcaConfirmCallback;
 ...
 */
typedef Callback< void, HrWpanPhyEnumeration,uint8_t > PlmeEdConfirmCallback;
 ...
 */
typedef Callback< void, HrWpanPhyEnumeration,
                  HrWpanPibAttributeIdentifier,
                  HrWpanPhyPibAttributes* > PlmeGetAttributeConfirmCallback;
 ...
 */
typedef Callback< void, HrWpanPhyEnumeration > PlmeSetTRXStateConfirmCallback;
 ...
 */
typedef Callback< void, HrWpanPhyEnumeration,
                  HrWpanPibAttributeIdentifier > PlmeSetAttributeConfirmCallback;

/**
 * \ingroup lr-wpan
 *
 * Make HrWpanPhy a SpectrumPhy so we can enable the eventual modeling of
 * device interference
 */
...
public:
   ...
   */
  void PlmeGetAttributeRequest (HrWpanPibAttributeIdentifier id);
   ...
   */
  void PlmeSetTRXStateRequest (HrWpanPhyEnumeration state);
   ...
   */
  void PlmeSetAttributeRequest (HrWpanPibAttributeIdentifier id, HrWpanPhyPibAttributes* attribute);
   ...
   *
   * @param e pointer to HrWpanErrorModel to use
   */
  void SetErrorModel (Ptr<HrWpanErrorModel> e);

  /**
   * get the error model in use
   *
   * @return pointer to HrWpanErrorModel in use
   */
  Ptr<HrWpanErrorModel> GetErrorModel (void) const;
  ...
  typedef void (* StateTracedCallback)
    (const Time time,
     const HrWpanPhyEnumeration oldState, const HrWpanPhyEnumeration newState);
...
protected:
   ...
   */
  static const HrWpanPhyDataAndSymbolRates dataSymbolRates[7];
  /**
   * The preamble, SFD, and PHR lengths in symbols for the different PHY options.
   * See Table 19 and Table 20 in section 6.3 IEEE 802.15.4-2006
   */
  static const HrWpanPhyPpduHeaderSymbolNumber ppduHeaderSymbolNumbers[7];
...
private:
   ...
   */
  void ChangeTrxState (HrWpanPhyEnumeration newState);
   ...
   */
  HrWpanPhyOption GetMyPhyOption (void);
   ...
   */
  void CancelEd (HrWpanPhyEnumeration state);
   ...
   * \see class CallBackTraceSource
   */
  TracedCallback<Time, HrWpanPhyEnumeration, HrWpanPhyEnumeration> m_trxStateLogger;
   ...
   */
  Ptr<HrWpanErrorModel> m_errorModel;

  /**
   * The current PHY PIB attributes.
   */
  HrWpanPhyPibAttributes m_phyPIBAttributes;

  // State variables
  /**
   * The current transceiver state.
   */
  HrWpanPhyEnumeration m_trxState;

  /**
   * The next pending state to applied after the current action of the PHY is
   * completed.
   */
  HrWpanPhyEnumeration m_trxStatePending;
   ...
   * considered noise.
   */
  Ptr<HrWpanInterferenceHelper> m_signal;
   ...
   * due to interference.
   */
  std::pair<Ptr<HrWpanSpectrumSignalParameters>, bool>  m_currentRxPacket;
