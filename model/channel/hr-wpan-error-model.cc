...
namespace ns3 {
	...
	NS_LOG_COMPONENT_DEFINE("HrWpanErrorModel");

	NS_OBJECT_ENSURE_REGISTERED(HrWpanErrorModel);

	TypeId
		HrWpanErrorModel::GetTypeId(void)
	{
		static TypeId tid = TypeId("ns3::HrWpanErrorModel")
			.SetParent<Object>()
			.AddConstructor<HrWpanErrorModel>()
			;
		return tid;
	}

	HrWpanErrorModel::HrWpanErrorModel(void)
	{
	...
	double
		HrWpanErrorModel::GetChunkSuccessRate(double snr, uint32_t nbits) const
	{
		double ber = 0.0;
