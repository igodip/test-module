...
Ptr<SpectrumModel> g_HrWpanSpectrumModel; //!< Global object used to initialize the HrWpan Spectrum Model

/**
 * \ingroup lr-wpan
 * \brief Helper class used to automatically initialize the HrWpan Spectrum Model objects
 */
class HrWpanSpectrumModelInitializer
{
public:
  HrWpanSpectrumModelInitializer (void)
  {
    ...
    g_HrWpanSpectrumModel = Create<SpectrumModel> (bands);
  }

} g_HrWpanSpectrumModelInitializerInstance; //!< Global object used to initialize the HrWpan Spectrum Model

HrWpanSpectrumValueHelper::HrWpanSpectrumValueHelper (void)
{
  NS_LOG_FUNCTION (this);
  m_noiseFactor = 1.0;
}
...
Ptr<SpectrumValue>
HrWpanSpectrumValueHelper::CreateTxPowerSpectralDensity (double txPower, uint32_t channel)
{
  NS_LOG_FUNCTION (this);
  Ptr<SpectrumValue> txPsd = Create <SpectrumValue> (g_HrWpanSpectrumModel);
...
Ptr<SpectrumValue>
HrWpanSpectrumValueHelper::CreateNoisePowerSpectralDensity (uint32_t channel)
{
  NS_LOG_FUNCTION (this);
  Ptr<SpectrumValue> noisePsd = Create <SpectrumValue> (g_HrWpanSpectrumModel);
...
double
HrWpanSpectrumValueHelper::TotalAvgPower (Ptr<const SpectrumValue> psd, uint32_t channel)
{
  NS_LOG_FUNCTION (psd);
  double totalAvgPower = 0.0;

  NS_ASSERT (psd->GetSpectrumModel () == g_HrWpanSpectrumModel);
