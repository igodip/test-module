...
HrWpanMac::McpsDataRequest (McpsDataRequestParams params, Ptr<Packet> p)
{
  ...
  HrWpanMacHeader macHdr (HrWpanMacHeader::HRWPAN_MAC_DATA, m_macDsn.GetValue ());
  m_macDsn++;

  if (p->GetSize () > HrWpanPhy::aMaxPhyPacketSize - aMinMPDUOverhead)
    {
...
HrWpanMac::CheckQueue ()
{
  ...
  if (m_hrWpanMacState == MAC_IDLE && !m_txQueue.empty () && m_txPkt == 0 && !m_setMacState.IsRunning ())
    {
      TxQueueElement *txQElement = m_txQueue.front ();
      m_txPkt = txQElement->txQPkt;
      m_setMacState = Simulator::ScheduleNow (&HrWpanMac::SetHrWpanMacState, this, MAC_CSMA);
    }
}
  ...
  else
    {
      ...
      if (m_macPromiscuousMode)
      ...
      else
        {
          ...
          acceptFrame = (receivedMacHdr.GetType () != HrWpanMacHeader::HRWPAN_MAC_RESERVED);
          ...
          if (acceptFrame
              && (receivedMacHdr.GetType () == HrWpanMacHeader::HRWPAN_MAC_BEACON))
            {
          ...
          if (acceptFrame
              && ((receivedMacHdr.GetType () == HrWpanMacHeader::HRWPAN_MAC_DATA)
                  || (receivedMacHdr.GetType () == HrWpanMacHeader::HRWPAN_MAC_COMMAND))
          ...
          if (acceptFrame)
            {
              ...
              if ((receivedMacHdr.IsData () || receivedMacHdr.IsCommand ()) && receivedMacHdr.IsAckReq ()
                  && !(receivedMacHdr.GetDstAddrMode () == SHORT_ADDR && receivedMacHdr.GetShortDstAddr () == "ff:ff"))
                {
                  ...
                  if (m_hrWpanMacState == MAC_ACK_PENDING)
                    {
                      m_ackWaitTimeout.Cancel ();
                      PrepareRetransmission ();
                    }
                  else if (m_hrWpanMacState == MAC_CSMA)
                    {
                  ...
                  ChangeMacState (MAC_IDLE);
                  m_setMacState = Simulator::ScheduleNow (&HrWpanMac::SendAck, this, receivedMacHdr.GetSeqNum ());
                }
              ...
              else if (receivedMacHdr.IsAcknowledgment () && m_txPkt && m_hrWpanMacState == MAC_ACK_PENDING)
                {
                  HrWpanMacHeader macHdr;
                  ...
                  if (receivedMacHdr.GetSeqNum () == macHdr.GetSeqNum ())
                    {
                      ...
                      m_setMacState.Cancel ();
                      m_setMacState = Simulator::ScheduleNow (&HrWpanMac::SetHrWpanMacState, this, MAC_IDLE);
                    }
                  else
                    {
...
HrWpanMac::SendAck (uint8_t seqno)
{
  ...
  HrWpanMacHeader macHdr (HrWpanMacHeader::HRWPAN_MAC_ACKNOWLEDGMENT, seqno);
  HrWpanMacTrailer macTrailer;
          ...
          if (macHdr.IsAckReq ())
            {
              ...
              m_ackWaitTimeout = Simulator::Schedule (waitTime, &HrWpanMac::AckWaitTimeout, this);
              m_setMacState.Cancel ();
              m_setMacState = Simulator::ScheduleNow (&HrWpanMac::SetHrWpanMacState, this, MAC_ACK_PENDING);
              return;
            }
          else
            {
...
HrWpanMac::SetHrWpanMacState (HrWpanMacState macState)
{
  ...
  if (macState == MAC_IDLE)
    ...
    {
      NS_ASSERT (m_hrWpanMacState == MAC_IDLE || m_hrWpanMacState == MAC_ACK_PENDING);

      ChangeMacState (MAC_CSMA);
      m_phy->PlmeSetTRXStateRequest (IEEE_802_15_4_PHY_RX_ON);
    }
  else if (m_hrWpanMacState == MAC_CSMA && macState == CHANNEL_IDLE)
    {
      // Channel is idle, set transmitter to TX_ON
      ChangeMacState (MAC_SENDING);
      m_phy->PlmeSetTRXStateRequest (IEEE_802_15_4_PHY_TX_ON);
    }
  else if (m_hrWpanMacState == MAC_CSMA && macState == CHANNEL_ACCESS_FAILURE)
    {
      NS_ASSERT (m_txPkt);
