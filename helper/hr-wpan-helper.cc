 ...
 */
static void
AsciiHrWpanMacTransmitSinkWithContext (
 ...
 */
static void
AsciiHrWpanMacTransmitSinkWithoutContext (
  Ptr<OutputStreamWrapper> stream,
  Ptr<const Packet> p)
{
...
void
HrWpanHelper::EnableLogComponents (void)
{
  LogComponentEnableAll (LOG_PREFIX_TIME);
  LogComponentEnableAll (LOG_PREFIX_FUNC);
  LogComponentEnable ("HrWpanCsmaCa", LOG_LEVEL_ALL);
  LogComponentEnable ("HrWpanErrorModel", LOG_LEVEL_ALL);
  LogComponentEnable ("HrWpanInterferenceHelper", LOG_LEVEL_ALL);
  ...
  LogComponentEnable ("HrWpanSpectrumSignalParameters", LOG_LEVEL_ALL);
  LogComponentEnable ("HrWpanSpectrumValueHelper", LOG_LEVEL_ALL);
}

std::string
HrWpanHelper::HrWpanPhyEnumerationPrinter (HrWpanPhyEnumeration e)
{
  switch (e)
    {
...
std::string
HrWpanHelper::HrWpanMacStatePrinter (HrWpanMacState e)
{
  switch (e)
    {
    case MAC_IDLE:
...
void
HrWpanHelper::AddMobility (Ptr<HrWpanPhy> phy, Ptr<MobilityModel> m)
{
  phy->SetMobility (m);
}

NetDeviceContainer
HrWpanHelper::Install (NodeContainer c)
{
  ...
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); i++)
    {
      ...
      // address and panId. Right now they are hardcoded in HrWpanMac::HrWpanMac ()
...
Ptr<SpectrumChannel>
HrWpanHelper::GetChannel (void)
{
  return m_channel;
}

void
HrWpanHelper::SetChannel (Ptr<SpectrumChannel> channel)
{
  m_channel = channel;
}

void
HrWpanHelper::SetChannel (std::string channelName)
{
...
int64_t
HrWpanHelper::AssignStreams (NetDeviceContainer c, int64_t stream)
{
  ...
  for (NetDeviceContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
      netDevice = (*i);
      Ptr<HrWpanNetDevice> lrwpan = DynamicCast<HrWpanNetDevice> (netDevice);
      if (lrwpan)
        {
...
void
HrWpanHelper::AssociateToPan (NetDeviceContainer c, uint16_t panId)
{
  NetDeviceContainer devices;
  uint16_t id = 1;
  uint8_t idBuf[2];
  ...
  for (NetDeviceContainer::Iterator i = c.Begin (); i != c.End (); i++)
    {
      Ptr<HrWpanNetDevice> device = DynamicCast<HrWpanNetDevice> (*i);
      if (device)
        {
          idBuf[0] = (id >> 8) & 0xff;
          idBuf[1] = (id >> 0) & 0xff;
 ...
 */
static void
PcapSniffHrWpan (Ptr<PcapFileWrapper> file, Ptr<const Packet> packet)
{
  file->Write (Simulator::Now (), packet);
}

void
HrWpanHelper::EnablePcapInternal (std::string prefix, Ptr<NetDevice> nd, bool promiscuous, bool explicitFilename)
{
  ...
  if (promiscuous == true)
    {
      device->GetMac ()->TraceConnectWithoutContext ("PromiscSniffer", MakeBoundCallback (&PcapSniffHrWpan, file));

    }
  else
    {
      device->GetMac ()->TraceConnectWithoutContext ("Sniffer", MakeBoundCallback (&PcapSniffHrWpan, file));
    }
}

void
HrWpanHelper::EnableAsciiInternal (
  Ptr<OutputStreamWrapper> stream,
  std::string prefix,
  Ptr<NetDevice> nd,
  bool explicitFilename)
{
  ...
  if (stream == 0)
    {
      ...
      device->GetMac ()->TraceConnectWithoutContext ("MacTx", MakeBoundCallback (&AsciiHrWpanMacTransmitSinkWithoutContext, theStream));

      asciiTraceHelper.HookDefaultEnqueueSinkWithoutContext<HrWpanNetDevice> (device, "MacTxEnqueue", theStream);
      asciiTraceHelper.HookDefaultDequeueSinkWithoutContext<HrWpanNetDevice> (device, "MacTxDequeue", theStream);
      asciiTraceHelper.HookDefaultDropSinkWithoutContext<HrWpanNetDevice> (device, "MacTxDrop", theStream);
  ...
  oss.str ("");
  oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid << "/$ns3::HrWpanNetDevice/Mac/MacRx";
  device->GetMac ()->TraceConnect ("MacRx", oss.str (), MakeBoundCallback (&AsciiTraceHelper::DefaultReceiveSinkWithContext, stream));

  oss.str ("");
  oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid << "/$ns3::HrWpanNetDevice/Mac/MacTx";
  device->GetMac ()->TraceConnect ("MacTx", oss.str (), MakeBoundCallback (&AsciiHrWpanMacTransmitSinkWithContext, stream));

  oss.str ("");
  oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid << "/$ns3::HrWpanNetDevice/Mac/MacTxEnqueue";
  device->GetMac ()->TraceConnect ("MacTxEnqueue", oss.str (), MakeBoundCallback (&AsciiTraceHelper::DefaultEnqueueSinkWithContext, stream));

  oss.str ("");
  oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid << "/$ns3::HrWpanNetDevice/Mac/MacTxDequeue";
  device->GetMac ()->TraceConnect ("MacTxDequeue", oss.str (), MakeBoundCallback (&AsciiTraceHelper::DefaultDequeueSinkWithContext, stream));

  oss.str ("");
  oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid << "/$ns3::HrWpanNetDevice/Mac/MacTxDrop";
